...
def identify_liquidity_sweeps(df, swing_points):
    ...
    for _, swing in swing_points.iterrows():
        ...
        subsequent_candles = df[df['Open Time'] >
                                swing['Date']].reset_index(drop=True)

        if swing['Type'] == 'Swing High':
            # Cumulative max of 'High' prices after the swing point
            cum_high_below_swing = subsequent_candles['High'].cummax(
            ) <= swing['Price']

            # Find the first index where the cumulative max exceeds the swing price
            first_breach_idx = cum_high_below_swing.idxmin(
            ) if not cum_high_below_swing.all() else None
            ...
            cum_low_above_swing = subsequent_candles['Low'].cummin(
            ) >= swing['Price']

            # Find the first index where the cumulative min goes below the swing price
            first_breach_idx = cum_low_above_swing.idxmin(
            ) if not cum_low_above_swing.all() else None
